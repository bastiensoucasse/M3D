...
void Mesh::createMesh(int M, int N)
{
    mVertices.resize(M * N);
    for (int j = 0; j < N; j++)
        for (int i = 0; i < M; i++)
            mVertices[j * M + i] = Vertex(Vector3f(i - M / 2, 0, j - N / 2));

    for (int j = 0; j < N - 1; j++)
        for (int i = 0; i < M - 1; i++) {
            mFaces.push_back(Vector3i(j * M + i, (j + 1) * M + i, j * M + i + 1));
            mFaces.push_back(Vector3i(j * M + i + 1, (j + 1) * M + i, (j + 1) * M + i + 1));
        }
}
