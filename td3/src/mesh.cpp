...
void Mesh::draw(const Shader& shd)
{
    ...
    int vertex_col = shd.getAttribLocation("vtx_color");
    if (vertex_col >= 0) {
        glVertexAttribPointer(vertex_col, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)(2 * sizeof(Vector3f)));
    ...
    if (vertex_col >= 0)
        glDisableVertexAttribArray(vertex_col);
