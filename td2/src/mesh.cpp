...
bool Mesh::intersectFace(const Ray &ray, Hit &hit, const int faceId) const
{
    ...
    const Point3f A = vertexOfFace(faceId, 0).position, B = vertexOfFace(faceId, 1).position, C = vertexOfFace(faceId, 2).position;
    const Vector3f E1 = A - C, E2 = B - C, T = O - C, P = D.cross(E2), Q = T.cross(E1);

    const float k = 1 / P.dot(E1);

    const float t = k * Q.dot(E2);
    if (t < 0)
        return false;

    const float u = k * P.dot(T);
    if (u < 0)
        return false;

    const float v = k * Q.dot(D);
    if (v < 0 || u + v >= 1)
        return false;

    // const Point3f x = u * E1 + v * E2 + C;

    const Normal3f An = vertexOfFace(faceId, 0).normal, Bn = vertexOfFace(faceId, 1).normal, Cn = vertexOfFace(faceId, 2).normal;
    const Normal3f normal = (u * (An - Cn) + v * (Bn - Cn) + Cn).normalized();
